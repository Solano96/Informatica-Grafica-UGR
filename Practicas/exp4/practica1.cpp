// *********************************************************************
// **
// ** Informática Gráfica, curso 2016-17
// ** Práctica 1  (implementación)
// **
// *********************************************************************

#include <cmath>
#include "aux.hpp"
#include "tuplasg.hpp"   // Tupla3f
#include "practicas.hpp"
#include "practica1.hpp"

using namespace std;

Cubo *cubo = NULL;
Tetraedro *tetraedro = NULL;

Cubo::Cubo(){
	float arista = 1.0;
	vertices.push_back(Tupla3f(-arista/2, -arista/2, -arista/2));     
	vertices.push_back(Tupla3f(arista/2, -arista/2, -arista/2));    
	vertices.push_back(Tupla3f(arista/2, arista/2, -arista/2));   
	vertices.push_back(Tupla3f(-arista/2, arista/2, -arista/2));
  
	vertices.push_back(Tupla3f(-arista/2, -arista/2, arista/2));
	vertices.push_back(Tupla3f(arista/2, -arista/2, arista/2)); 
	vertices.push_back(Tupla3f(arista/2, arista/2, arista/2)); 
	vertices.push_back(Tupla3f(-arista/2, arista/2, arista/2));

	caras.push_back(Tupla3i(0,1,2));
	caras.push_back(Tupla3i(0,2,3));
	caras.push_back(Tupla3i(3,0,4));
	caras.push_back(Tupla3i(7,4,3));
	caras.push_back(Tupla3i(5,6,7));
	caras.push_back(Tupla3i(4,5,7));
	caras.push_back(Tupla3i(5,6,1));
	caras.push_back(Tupla3i(1,2,6));
	caras.push_back(Tupla3i(0,1,5));
	caras.push_back(Tupla3i(4,5,0));
	caras.push_back(Tupla3i(2,3,7));
	caras.push_back(Tupla3i(6,7,2));
	
	num_vert = 8;
	num_caras = 12;

	nombre_obj = "Cubo";

	CrearParesImpares();
}

Tetraedro::Tetraedro(){
	vertices.push_back(Tupla3f(0, sqrt(6)/4, 0));     
	vertices.push_back(Tupla3f(0, sqrt(6)/4-sqrt(6)/3, sqrt(3)/3));    
	vertices.push_back(Tupla3f(-0.5, sqrt(6)/4-sqrt(6)/3, sqrt(3)/3-sqrt(3)/2));   
	vertices.push_back(Tupla3f(0.5, sqrt(6)/4-sqrt(6)/3, sqrt(3)/3-sqrt(3)/2)); 

	caras.push_back(Tupla3i(1,2,3));
	caras.push_back(Tupla3i(0,1,2));
	caras.push_back(Tupla3i(0,2,3));
	caras.push_back(Tupla3i(0,3,1));	

	num_vert = 4;
	num_caras = 4;

	nombre_obj = "Tetraedro";

	CrearParesImpares();
}


unsigned objeto_activo = 0 ; // objeto activo: cubo (0), tetraedro (1), otros....

// ---------------------------------------------------------------------
// declaraciones de estructuras de datos....



// ---------------------------------------------------------------------
// Función para implementar en la práctica 1 para inicialización.
// Se llama una vez al inicio, cuando ya se ha creado la ventana e
// incializado OpenGL.

void P1_Inicializar( int argc, char *argv[] )
{
	cubo = new Cubo();
	tetraedro = new Tetraedro();
}

// ---------------------------------------------------------------------
// Función invocada al pulsar una tecla con la práctica 1 activa:
// (si la tecla no se procesa en el 'main').
//
//  - devuelve 'true' si la tecla se usa en esta práctica para cambiar
//    entre el cubo, el tetraedro u otros objetos (cambia el valor de
//    'objeto_activo').
//  - devuelve 'false' si la tecla no se usa en esta práctica (no ha
//    cambiado nada)

bool P1_FGE_PulsarTeclaNormal( unsigned char tecla ){
  
	if(tecla == 'O'){
		objeto_activo++;
		objeto_activo%=2;
		return true;
	}

	return false ;
}


// ---------------------------------------------------------------------
// Función a implementar en la práctica 1  para dibujar los objetos
// se debe de usar el modo de dibujo que hay en el parámetro 'cv'
// (se accede con 'cv.modoVisu')

void P1_DibujarObjetos( ContextoVis & cv )
{
	if(objeto_activo == 0)
		cubo->visualizarGL(cv);
	else
		tetraedro->visualizarGL(cv);

}
