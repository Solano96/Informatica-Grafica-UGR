...
void FijarCamara()
{
   ...
   /*Matriz4f matrizModelado = MAT_Ident();
...
void DibujarObjetos()
{
   switch( practicaActual )
   {
      ...
      case 4:
         ...
         break;
      case 5:
         P5_DibujarObjetos(contextoVis) ; // definido en 'practica4.hpp'
         break;
      default :
   ...
   switch (toupper(tecla))
   {
      ...
      case '-' :
         if (practicaActual == 5)
            ...
            frustum_factor_escala /= 1.05;
         break;
		// Cambiar modo de visualización tecla "M"
		case 'M' :
			if(contextoVis.modoVisu == modoPuntos)
				...
				contextoVis.modoVisu = modoColor;
         else if(contextoVis.modoVisu == modoColor)
            contextoVis.modoVisu = modoSombreadoPlano;
         else if(contextoVis.modoVisu == modoSombreadoPlano)
            contextoVis.modoVisu = modoSombreadoSuave;
         else 
            contextoVis.modoVisu = modoPuntos;
			break;
		// Cambiar práctica actual tecla "P" 
		case 'P':
			practicaActual+=num_practicas;
			practicaActual%=(num_practicas);
			practicaActual++;
		...
		break;
		// Cambiar a modo diferido o a modo inmediato
		case 'V':
         if (practicaActual == 5)
            P5_FGE_PulsarTeclaNormal(toupper(tecla));
         else{
   			if(contextoVis.modoDiferido){
   				contextoVis.modoDiferido = false;
   				cout << "Desactivado modo diferido." << endl;
   			}
   			else{
   				contextoVis.modoDiferido = true;
   				cout << "Activado modo diferido" << endl;			
   			}
         }
		break;
      default:
         ...
         {
            case 1 :
               redibujar = P1_FGE_PulsarTeclaNormal( toupper(tecla) ) ; // true si es necesario redibujar
               break ;
				case 2 :
					redibujar = P2_FGE_PulsarTeclaNormal( toupper(tecla) ) ; // true si es necesario redibujar
					break;
            case 3 :
               redibujar = P3_FGE_PulsarTeclaNormal( toupper(tecla) ) ; // true si es necesario redibujar

               if(redibujar && toupper(tecla) == 'A')
                  glutIdleFunc(FGE_Desocupado);
               
               break;
            case 4 :
               redibujar = P4_FGE_PulsarTeclaNormal( toupper(tecla) ) ;                
               break;
            case 5 :
               redibujar = P5_FGE_PulsarTeclaNormal( toupper(tecla) ) ; 
               break;
            default :
...
void FGE_PulsarTeclaEspecial( int tecla, int x_raton, int y_raton )
{
   ...
   if(practicaActual == 5){
      P5_FGE_PulsarTeclaEspecial(tecla);
   }
   else{
      switch ( tecla )
      {
