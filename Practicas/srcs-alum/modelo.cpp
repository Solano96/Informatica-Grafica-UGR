...
Disco::Disco(){
	...
	float inc = 2.0*pi*1.0/n;
	float a = 0;

	for(int i = 0; i < n; i++){
		vertices.push_back(Tupla3f(0,0,0));
		coords_text.push_back(Tupla2f(0.5,0.5));
		vertices.push_back(Tupla3f(cos(a),0,sin(a)));
		coords_text.push_back(Tupla2f(cos(a)/2.0+0.5,sin(a)/2.0+0.5));
		a +=inc;
		vertices.push_back(Tupla3f(cos(a),0,sin(a)));
		coords_text.push_back(Tupla2f(cos(a)/2.0+0.5,sin(a)/2.0+0.5));
		caras.push_back(Tupla3i(i*3,i*3+1,i*3+2));
	}
...
Moneda::Moneda(){
	agregar(new MaterialMoneda());
	agregar(MAT_Escalado(1,0.1,1));	
	agregar(new Disco());
	agregar(new MaterialBarra());
	agregar(new Cilindro1());
...
Escena::Escena(){
        ...
        agregar(MAT_Traslacion(-6,-1.4,3));
        agregar(new Moneda());     
        agregar(MAT_Traslacion(3,0,0));        
        agregar(MAT_Rotacion(20, 1,0,0));
        agregar(new Moneda());     
        agregar(MAT_Traslacion(3,0,0));
        agregar(MAT_Rotacion(20, 1,0,0));
        agregar(new Moneda());
